...
Ambient movement is isolated from body morphing, expression morphing, gaze, blinking, and face turning. The rig follows the supplied reference's spherical projection: each eye travels around a virtual face, compresses with depth, and is clipped against the live silhouette as it reaches an edge. The same clip is embedded in portable SVG exports, so extreme poses never draw outside the character. All eight rig values are continuous and retarget interruptible springs from the pose already on screen. Each of the twelve face performances combines a curated 4–5-expression timeline with additive multi-keyframe rig motion. The gestures visibly change turn, spacing, scale, gaze, tilt, height, or depth, and always return to the editable base pose; the rig panel mirrors live performance values and clicking the active performance replays it. Twenty-five contours and data-driven state pools keep the expression space extensible without animation-specific UI branches. Automatic expression changes pause during a performance, otherwise run at a restrained, state-specific cadence, and stop under `prefers-reduced-motion`. Each animation layer uses transforms, opacity, color, or the SVG path itself; the live rig meter is a transform-only progress overlay.
